...
precio_kwh = 0.0
# NUEVO: Estado de pausa administrativa
is_paused = False 
...
def start_supply_simulation_thread():
    ...
    while True:
        # MODIFICADO: Solo sumar si esta activo Y NO ESTÁ PAUSADO
        if current_charge["active"] and kafka_producer and not is_paused:
...
def user_interface_thread():
    ...
    while True:
        try:
            ...
            _original_print(f"Kafka: {UI_KAFKA_ADDR} | Monitor: {UI_MONITOR_ADDR}")
            _original_print("----------------------------------------")
            _original_print(f"{Colors.BOLD}ESTADO: {Colors.RESET}", end="")
            
            # MODIFICADO: Mostrar estado de Pausa
            if paused:
            ...
            _original_print(f"{Colors.RED}SIMULADA{Colors.RESET}" if averia else f"{Colors.GREEN}OK{Colors.RESET}")
            _original_print("----------------------------------------")
            ...
            _original_print("----------------------------------------")
            _original_print("[a] Avería | [r] Resolver | [e] Enchufar | [d] Desenchufar")
            
            comando = input(f"\n{Colors.BOLD}> {Colors.RESET}").strip().lower()
            
            if comando == 'a':
                simular_averia = True
                print(f"{Colors.RED}[!!!] AVERÍA SIMULADA.{Colors.RESET}")
                if current_charge["active"]:
                    ...
                    if kafka_producer: kafka_producer.send('CPTelemetry', f"TICKET:{cp_id}:{current_charge['driver_id']}:{kwh}:{cost}:AVERIA")
                    current_charge["driver_id"] = None

            elif comando == 'r':
                simular_averia = False
                print(f"{Colors.GREEN}[OK] AVERÍA RESUELTA.{Colors.RESET}")

            elif comando == 'e':
                if charge_request_pending and not is_paused:
                    charge_request_pending = False
                ...
                if current_charge["active"] or is_paused: # Permitir desenchufar si está pausado
                    ...
                    current_charge["active"] = False
                    current_charge["driver_id"] = None
                    # Si desenchufamos manualmente, quitamos la pausa también para el siguiente
                    is_paused = False 
...
def start_kafka_consumer_thread():
    ...
    try:
        ...
        for message in consumer:
            ...
            cmd = parts[0]
            if cmd == "START" and len(parts) == 3 and not current_charge["active"]:
            ...
            elif cmd == "STOP": # Stop forzado (no pausa)
...
def handle_monitor_connection(conn, addr):
    ...
    with UI_LOCK: UI_MONITOR_CLIENT = str(addr)
    try:
        while True:
            if not conn.recv(1024): break
            # MODIFICADO: Responder con estado de PAUSA si aplica
            if simular_averia: conn.sendall(b"KO: AVERIA SIMULADA")
            elif is_paused: conn.sendall(b"OK:PAUSED") # Informar al monitor de la pausa
            else: conn.sendall(b"OK")
    except: pass
    finally:
        with UI_LOCK: UI_MONITOR_CLIENT = "N/A"
        conn.close()
