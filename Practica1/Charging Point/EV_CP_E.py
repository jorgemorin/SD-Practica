...
def wait_for_monitor(cp_id):
    """
    Engine espera una conexión directa del monitor (EV_CP_M).
    El monitor debe validar su ID con Central antes de llegar aquí.
    """
    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s_monitor:
            ...
            s_monitor.listen(1)
            print(f"[ENGINE] Esperando conexión del Monitor (CP_ID={cp_id}) en puerto {PORT + 1} ...")

            conn, addr = s_monitor.accept()
            with conn:
                print(f"[ENGINE] Monitor conectado desde {addr}")

                # Recibir mensaje de autenticación del monitor
                ...
                if mensaje == f"MONITOR:{cp_id}":
                    ...
                    print("[ENGINE] ID de monitor no válido.")
